...
void ImuCallback(const axon_link::Imu::ConstPtr& imu_data)
{
...
int main(int argc, char** argv)
{
    ...
    IMU_sub_= n.subscribe("/fcu/imu", 10, ImuCallback);
    ...
    while(ros::ok())
    {
        ...
        if ( (rx_ec_flag==1) && (imu_rev_flag==1) )
        {
            ...
            rc.v_L=(rm.r/2.0) *(rc.w_Rs+rc.w_Ls);
            rc.w_L=(rm.r/rm.L)*(rc.w_Rs-rc.w_Ls);
            if (rc.w_L>=0.02)
                th=imu_pose.yaw;
