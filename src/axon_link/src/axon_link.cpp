...
int rx_ec_flag=0;
unsigned int rev_ec_count=0;
unsigned int err_ec_count=0;
double ec_dt=0;
...
unsigned char AXON_Wheel_Motor_Speed(double w_X)
{
    ...
    else
    {
        ...
        //--Limit Wheel Speed--------------------------------------------------
        Sat_Value(&w_X, AXON_WHEEL_MAXSPEED, 0);
        if (w_X>=AXON_WHEEL_MINSPEED) //min Speed (rad/s)
            w_ui=(unsigned int)(16.622*w_X + 0.1538 ); //min speed:0.0065, max speed:7.5
        else
            w_Xa=0;

        //--AXON CMD format(0x00~0xFF)------------------------------------------
...
void Packet_Decode(void)
{
    ...
    if (rx_stop_flag==0)
    {
        ...
        if (rx_timeout_flag==0)
        {
            ...
            rx_count++;
            if (rx_count>=RX_LEN)//CMD1 reponse (2bytes) + CMD3 reponse (9 bytes)
            {
                ...
                if ( (rx_buff[0]==0x01) && (rx_buff[1]==0x01) && (rx_buff[2]==0x03) )
                {
                    ...
                    left_ec=(long int)(rx_buff[3]<<24);
                    left_ec=left_ec+(long int)(rx_buff[4]<<16);
                    left_ec=left_ec+(long int)(rx_buff[5]<<8);
                    left_ec=left_ec+(long int)(rx_buff[6]);

                    right_ec=(long int)(rx_buff[7]<<24);
                    right_ec=right_ec+(long int)(rx_buff[8]<<16);
                    right_ec=right_ec+(long int)(rx_buff[9]<<8);
                    right_ec=right_ec+(long int)(rx_buff[10]);

                    //--Calculation two wheel-------------------------------------
                    current_time = ros::Time::now();
                    double dt = (current_time - last_time).toSec();
                    total_time+=dt;
                    rc.w_Ls=((double)left_ec*RAD_PLUSE)/dt;
                    rc.w_Rs=((double)right_ec*RAD_PLUSE)/dt;

                    rev_ec_count++;
                    if ( (fabs(rc.w_Ls)<=AXON_WHEEL_MAXSPEED) && (fabs(rc.w_Rs)<=AXON_WHEEL_MAXSPEED) )
                    {
                    ...
                    if (debug_flag_)
                        printf("L_Encoder=%ld, R_Encoder=%ld, rc.w_L=%+3.3f, rc.w_R=%+3.3f, err=%d/%d\n" , left_ec, right_ec, rc.w_L, rc.w_R, err_ec_count, rev_ec_count);

                    printf("Ttime=%+3.3f, W_LSp=%+3.3f, W_RSp=%+3.3f, AvgLWSp=%+3.3f, AvgR_WSp=%+3.3f, dt=%+3.3f\n" , total_time, rc.w_Ls, rc.w_Rs, rc.avg_w_Ls, rc.avg_w_Rs, dt);
...
{    
    ...
    while(ros::ok())
    {
        //--test wheel speed------------------
        //cmd_vel_hold_cnt=0;
        //rc.w_L=0.1; //now limit to -3 ~0~ +3, min-max:-7.5 ~0 ~ +7.5 (rad/s)
        //rc.w_R=0.8; //now limit to -3 ~0~ +3, min-max:-7.5 ~0 ~ +7.5 (rad/s)
        ...
        if (use_rev_encode_==1)
        {
            ...
            if (rx_ec_flag==1)
            {
                ...
                rc.v_L=(rm.r/2.00)*(rc.w_Rs+rc.w_Ls); //(From two wheel velocity)
                ...
                printf("x=%+3.3f, y=%+3.3f, th=%+3.3f, th=%+3.3f(deg), Yaw(IMU)=%+3.3f\n", x, y, th, th*RTD, imu_pose.yaw);
